...
設定コンソールが書き換えるファイルは、次の 6 ファイルだけである。
...
「GPT 系サブエージェント経路を有効にする (impl-hard / impl-standard / impl-light)」は、GPT 側の 3 定義に書く `codex_enabled` の値を切り替える。
...
6 ファイルのいずれかが無い場合、またはフロントマターが壊れていて読めない場合は、その理由を画面に示し、「保存」を無効にする。
...
GPT 側の `codex_model` は、空でもエラーにしない。空はその区分で GPT 側を使わない設定として扱われるため、GPT 経路が有効かどうかにかかわらず検証しない。
