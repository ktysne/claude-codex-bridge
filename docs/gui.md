...
「はい」を選ぶと入力中の値を保持したまま定義ファイルを読み直し、「いいえ」を選ぶと入力を捨てて読み直す。
「キャンセル」を選ぶと何もしない。
...
取得中は「取得中...」と表示する。
`codex_home` が読めない場合は「認証ホーム未設定のため取得しない。既定値を使用」と表示する。
...
「保存」は、定義ファイルが揃っていて、未保存の変更か `codex_enabled` の不正値の修復待ちがあるときだけ押せる。
...
`codex_enabled` の不正値の修復待ちがあり、他に変更がないときは「保存すると codex_enabled の不正値を直します。」と表示する。
