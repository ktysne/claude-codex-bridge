...
画面は「サブエージェント」と「レビューと実装補助」の 2 タブで構成する。
対象と再読込、注意書き、保存状態、保存と閉じるのボタンはタブの外にあり、どちらを開いていても同じ位置に表示する。
...
保存すると、不正値が書かれた定義だけを表示どおりの値へ直す。正常な値が書かれた他の定義には触れない。
そのため、一部が不正値で残りが `true` だった場合は、修復した結果として 3 定義の値が食い違う。この食い違いは、次に述べる手順で解消する。

3 定義の `codex_enabled` が食い違っている場合は、3 つすべてが有効なときだけ有効として表示する。
食い違いは画面に示す。定義ファイルが無い、読めない、`codex_enabled` が不正値である、のいずれかを示しているときは、そちらを先に示す。
一部だけ無効の状態を有効として表示すると、別の項目を保存したときに無効側が有効へ戻るためである。
...
| `codex-subagent` | 編集 | 編集 | 表示のみ | 表示のみ |

GPT モデルの選択肢に「(未設定)」はない。
`codex_home` と `codex_sandbox` を編集できない理由は、[設計文書の「レビューと実装補助タブで書き換えない項目」](gui-console-design.md#レビューと実装補助タブで書き換えない項目)を参照する。
`codex --version` と GPT モデル一覧は、各定義に書かれた `codex_home` ごとに取得する。
2 定義が同じホームを使う場合は取得を 1 回にまとめ、結果もホーム別に保持する。
状態行には 2 つのホームの結果を並べ、両定義のホームが同じ場合は 1 つにまとめる。
...
設定コンソールが決めるのは、`impl-hard`、`impl-light`、`impl-standard` がどの認証ホームを使うかだけである。
どのホームをどの用途に割り当てるかは [CLAUDE.md](../CLAUDE.md) の「用途固定の原則」が定める。
利用上限を避けるためにホームを切り替える使い方はしない。

選んだ値は、保存すると `impl-hard`、`impl-light`、`impl-standard` の 3 つの `codex_home` に書き込まれる。
3 定義の値が食い違っている場合は、`impl-light` の値を選択中として表示し、食い違いを画面に示す。
このとき保存すると、選択中の値で 3 つが揃う。
...
「保存」は、対象ファイルが揃ったタブに未保存の変更か修復待ちがあるときだけ押せる。
一方のタブの対象ファイルが欠けているか読めなくても、もう一方のタブは単独で保存できる。
...
保存の前に、対象が揃っているタブのファイルがまだ存在することを確認する。読み込み後に消えたファイルがあれば、もう一方のタブに変更があっても保存を中断する。この場合は再読込してから保存する。
...
サブエージェントタブでは選択中の認証ホームを使い、レビューと実装補助タブでは各定義の `codex_home` を使う。
`CODEX_HOME` には、取得対象の認証ホームを展開した値を渡す。
レビューと実装補助タブでは、2 定義の `codex_home` が同じ場合に取得を 1 回だけ行う。
取得結果はホームごとに保持し、同じホームを使う行で共有する。
...
サブエージェントタブの `codex_home` は切り替えられるが、選べるのは実在する `.codex` で始まるディレクトリだけである。
レビューと実装補助タブでは、2 定義の `codex_home` と `codex_enabled`、Claude 側の `model` は編集対象にしない。
理由は[設計文書の「レビューと実装補助タブで書き換えない項目」](gui-console-design.md#レビューと実装補助タブで書き換えない項目)を参照する。
