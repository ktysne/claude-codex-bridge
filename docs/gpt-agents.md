...
5 つのサブエージェントは依頼を受けると、依頼文の全文を Write ツールでファイルに書く。
そのファイルを標準入力へリダイレクトして、ユーザ側の `~/.claude/tools/codex-agent.sh` を 1 回呼ぶ。
コマンド文字列(引数やヒアドキュメント)に埋め込むと、Bash ツールの加工で 8,191 文字を超える依頼文が構文エラーになり、`\\` が `\` に変わるためである。
置き場はセッションのスクラッチパッドで、示されていないときだけ `~/.claude/codex-agent/prompts/` を使う(7 日を過ぎたものはラッパーが消す)。
