...
その後、Bash ツールがバックグラウンドへ移したコマンドが終わると、サブエージェントは自動で再開し、同じエージェントの通知をもう一度送って最終報告を返す。
...
案内の文面には完了の判定方法も含める。記録には再開を確かめられない例も少数あり、その原因は分かっていないためである。
状態を確かめるときは、ログまたは出力ファイルの最後の行が `codex-agent: result=` の行かどうかで完了を判定する。
