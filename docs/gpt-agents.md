...
複数のプロジェクトにまたがる変更を委譲するときは、依頼文の作業ディレクトリに共通の親(たとえば `D:/Desktop/Develop`)を指定する。
2026-09-03 に、親を指定せずに 15 プロジェクトの `CLAUDE.md` 追記を依頼したところ、呼び出し元リポジトリ以外はすべて書き込み拒否になった。
...
2026-09-03 に 9 プロジェクトの追記を 1 回で依頼したところ、7 プロジェクトまで書いたところで打ち切られた。
