...
GPT への委譲を止めて Claude 側で実装させたいときは、依頼文に次の 1 行を置き、続けて理由を書く。

```text
委譲: Claude 側で実装
```

この行がある依頼では、`impl-hard`、`impl-light`、`impl-standard` は `codex-agent.sh` を呼ばずに自分で実装し、報告の冒頭に「委譲の指定により Claude 側で実装した」と書いて、指定の行と理由を添える。
指定として認めるのはこの 1 行の書き方だけで、「Codex を使わずに」のような言い回しは指定として扱わない。
