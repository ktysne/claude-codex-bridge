...
そのため、案内の文面には、次の通知が届かない場合の状態の確かめ方も含める。
状態を確かめる必要があるときは、報告に添えた出力ファイルまたはログの最後の行を 1 回だけ読む。
最後の行が `codex-agent: result=` の行なら、Codex は終わっている。次の通知が届いていなければ、そのファイルにある最終報告を結果として扱う。
`result=` の行が無ければ実行中なので、待ち続ける。委譲をやり直すのは、残った Codex の実行が無いことを確かめてからにする。
ログが空でないことは、完了の根拠にならない。ログの 1 行目は Codex の起動前に書かれるためである。
