...
- **保存**：5 ファイルが揃っていて、未保存の変更があるときだけボタンを押せる。検証に通れば 5 ファイル(Claude 側 3 つ、GPT 側 2 つ)を書き換え、値が変わっていないファイルには触れない。状態行は「未保存の変更があります」「変更はありません」「保存しました。書き換えたファイル: ...」「保存しました。変更はありません」のいずれかを常に示し、検証エラーと保存失敗は赤色で示す。照合に失敗した場合は保存せず、入力を保持する再読込を促すダイアログを出す。
