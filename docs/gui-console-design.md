...
実装は未着手であり、この文書は実装者への仕様書として書く。
実装に着手する前に、末尾の「実装の段階」に従ってブランチと PR を分ける。
...
- **保存**：検証に通れば 5 ファイル(Claude 側 3 つ、GPT 側 2 つ)を書き換え、値が変わっていないファイルには触れない。書き換えたファイル名を状態行に示す。
...
- キーの値は `<キー>:` の後ろの空白を除いた部分とする。「空白 + `#`」以降は行内コメントとして扱い、値には含めない。値を囲む `"` または `'` は外す。
...
## 実装開始時に Opus 5 へ渡す依頼文

別セッションで実装を始めるときは、次の文をそのまま最初のメッセージとして渡す。
段階 1 から始め、段階ごとに PR を分ける前提で書いてある。

```text
docs/gui-console-design.md に従って、設定コンソール(Windows GUI)の実装を段階 1 から始めてください。

前提:
- 作業前に CLAUDE.md、docs/gui-console-design.md、docs/gpt-agents.md、tools/codex-agent.sh を読む。
- main ブランチでは作業せず、段階ごとにブランチと PR を分ける(段階 1: feat/codex-enabled-flag、段階 2: feat/gui-frontmatter-lib、段階 3: feat/gui-console)。
- 設計書と実装が食い違ったら、設計書の意図を優先し、食い違いを PR 本文の「備考」に残す。設計書そのものを直す必要があるときは同じ PR で直す。
- 実装はメインセッションで直接行わず、設計書の「実装の段階」に書いた変更対象、期待する結果、検証方法をそのまま依頼文にして impl-standard に切り出す。監査と検証はメインセッションが行う。
- 段階 1 の検証で codex_enabled: true と省略時を確認する実行は利用枠を消費する。false の確認を先に行い、true と省略時の確認は各 1 回にとどめる。
- コミットメッセージ、PR 本文、コメントは日本語で書く。PR 本文は CLAUDE.md の記載ルール(概要、背景・目的、変更内容、影響範囲、改修後に行った確認内容、確認して欲しいポイント)に従う。
- 各段階の PR を作成した時点で、AskUserQuestion でクロスレビューの 3 択(依頼する / 依頼と修正適用 / 行わない)を提示して止まる。次の段階へは指示を待ってから進む。

段階 1 の完了条件:
- .claude/gpt-agents/impl-light.md に codex_enabled: false を一時的に書いた状態で tools/codex-agent.sh impl-light を実行すると、Codex を起動せず終了コード 3 と `codex-agent: result=failed exit=3` を返す。
- 不正値(例: codex_enabled: yes)では終了コード 2 で止まる。
- docs/gpt-agents.md のキー一覧と終了コード 3 の説明、impl-light と impl-standard の Claude 側定義の文言が設計書どおりに更新されている。
- 一時的な書き換えは元に戻してある。
```
