...
3. 終了コード 3(GPT 側が未導入、無効化、または未設定である)なら、以下の「Claude 側の進め方」に従って自分で実装する。報告の冒頭に「GPT 側が未導入、無効化、または未設定のため Claude 側で実装した」と書き、出力にある `codex-agent:` の理由行をそのまま添える。スクリプトが `~/.claude/tools/codex-agent.sh` に無い場合も、同じく未導入として扱う。
