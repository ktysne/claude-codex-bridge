...
- 検証コマンド: `bash -n tools/codex-agent.sh`、`node tools/cross-review.js --help`、`npm run test:metrics`、GUI は `dotnet build gui/CodexBridgeConsole.sln -c Release` と `dotnet test gui/CodexBridgeConsole.sln -c Release`。
