...
- 依頼文に git の語を入れない。担当はコミットや push を行わないので書く必要が無く、`isolation: "worktree"` で起動した担当では、git の語を含む依頼文の起動が隔離の検査で拒否される。
- 報告を受けたら、`codex-agent: result=` の行の有無と、Claude 側で実装した場合はその理由を確かめる。理由が依頼文の指示によるものなら、開発者に伝える。
